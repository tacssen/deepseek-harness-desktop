...
const API_KEY = credentialRef('BIGMODEL_API_KEY')
...
function parseImage(data) {
  ...
  if (!dimensions || dimensions.width < 20 || dimensions.height < 20 || dimensions.width > 6000 || dimensions.height > 6000) throw new Error('image dimensions must be between 20x20 and 6000x6000 pixels')
...
export function apply(ctx) {
  ctx.tools.register(defineTool({
    name: 'vision_analyze',
    description: 'Analyze a validated PNG/JPEG image through the independent GLM Vision bridge. Images never go to DeepSeek.',
    parameters: { data: { type: 'string', required: true, description: 'Base64 image data URL, <=5 MB.' }, prompt: { type: 'string', description: 'Short image question.' } },
    ...
    async execute(args, exec) {
      ...
      const image = parseImage(args.data)
      const imageHash = crypto.createHash('sha256').update(image.buffer).digest('hex')
      const endpoint = String(process.env.BIGMODEL_BASE_URL || DEFAULT_ENDPOINT).replace(/\/$/, '')
      const model = String(process.env.BIGMODEL_MODEL || DEFAULT_MODEL)
      const prompt = args.prompt || 'Describe this image briefly.'
      const cacheKey = crypto.createHash('sha256').update(JSON.stringify({ imageHash, prompt, provider: 'glm', model, endpoint })).digest('hex')
      const cached = cache.get(cacheKey)
      if (cached) return { ...cached, sha256: imageHash }
      const response = await fetch(endpoint, { method: 'POST', headers: { Authorization: `Bearer ${key.value}`, 'Content-Type': 'application/json' }, signal: exec.signal, body: JSON.stringify({ model, messages: [{ role: 'user', content: [{ type: 'text', text: prompt }, { type: 'image_url', image_url: { url: `data:${image.mime};base64,${image.buffer.toString('base64')}` } }] }], max_tokens: 256, stream: false }) })
