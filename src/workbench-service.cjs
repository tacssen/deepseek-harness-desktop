...
class WorkbenchService {
  ...
  async getSnapshot() {
    ...
    const base = { backend: { ready: Boolean(this.getPort()), port: this.getPort() || null }, workspace: { path: workspace, name: path.basename(workspace) }, session: { id: '', title: '', running: false, mode: 'chat' }, goal: null, todos: [], timeline: [], diffs: [], stats: {}, usage: this.usage?.summarize?.() || null, agentLevel: { ...effectiveLevel, levels: listAgentLevels(), providerNote: 'DeepSeek 官方原生 effort 当前为 high/max；低/中由 Desktop 执行预算区分。' }, permissions, browser: { enabled: Boolean(prefs.browser?.enabled && permissions.browser), headless: Boolean(prefs.browser?.headless) }, appearance: { ...(prefs.appearance || {}) }, skillCatalog: [], mcpConnections: [], checkpoints: [], problems: [], logs: [], gitDiff: '', terminalHistory: [], workspaceChanges: [...this.workspaceChanges].reverse(), shared };
